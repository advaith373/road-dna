...
const ConfidenceBar: React.FC<{ value: number }> = ({ value }) => {
  ...
  const color = pct >= 80 ? 'var(--critical)' : pct >= 60 ? 'var(--warning)' : 'var(--success)';
  return (
    <div style={{ marginTop: 4 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
        <span style={{ fontFamily: 'JetBrains Mono', fontSize: 8, color: 'var(--text-muted)', letterSpacing: '0.08em' }}>CONFIDENCE</span>
      ...
      <div style={{ height: 4, background: 'rgba(231,229,223,0.07)', position: 'relative', overflow: 'hidden' }}>
        <div style={{
          ...
          width: `${pct}%`,
          background: `linear-gradient(to right, var(--success) 0%, var(--amber) 60%, var(--critical) 100%)`,
      ...
      <div style={{ marginTop: 8, display: 'flex', alignItems: 'flex-end', gap: 2, height: 24 }}>
        {Array.from({ length: 20 }, (_, i) => {
          ...
          const barColor = i / 19 < 0.33 ? 'var(--success)' : i / 19 < 0.66 ? 'var(--amber)' : 'var(--critical)';
          return (
            <div key={i} style={{
              flex: 1,
...
const PredRow: React.FC<{ label: string; value: string; valueColor?: string }> = ({ label, value, valueColor = 'var(--text-primary)' }) => (
  <div style={{
    display: 'flex',
    ...
    <span style={{ fontFamily: 'Inter', fontSize: 10, color: 'var(--text-muted)', letterSpacing: '0.04em' }}>{label}</span>
...
export const PredictionPanel: React.FC = () => {
  ...
  const conditionColors: Record<string, string> = {
    smooth: 'var(--success)', rough: 'var(--warning)', bumpy: 'var(--amber)', potholed: 'var(--critical)', hazardous: '#ff1744',
  };
  const condColor = conditionColors[road.condition] || 'var(--text-secondary)';
    ...
    <div style={{
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <PredRow
          ...
          valueColor={road.potholeDetected ? 'var(--critical)' : road.roughness === 'high' ? 'var(--warning)' : 'var(--success)'}
        />
        <PredRow label="Distance" value={road.potholeDetected ? `${road.potholeDistance.toFixed(1)} m` : '—'} valueColor="var(--text-primary)" />
        <PredRow label="Estimated impact" value={road.potholeDetected ? `${road.potholeEta.toFixed(1)} s` : '—'} valueColor="var(--text-primary)" />
        ...
        <div style={{
          ...
          <div style={{ fontFamily: 'JetBrains Mono', fontSize: 8, color: 'var(--text-label)', letterSpacing: '0.1em', marginBottom: 4 }}>
            RECOMMENDED ACTION
          </div>
          <div style={{
            display: 'flex',
            ...
            <ChevronRight size={10} color={road.potholeDetected ? 'var(--critical)' : 'var(--amber)'} />
            <span style={{
              fontFamily: 'JetBrains Mono',
              fontSize: 11,
              fontWeight: 700,
              color: road.potholeDetected ? 'var(--critical)' : 'var(--amber)',
