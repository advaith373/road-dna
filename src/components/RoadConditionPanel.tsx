      ...
      <div style={{ flex: 1, padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 8, overflow: 'auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flexShrink: 0 }}>
            <svg width={72} height={44} style={{ overflow: 'visible' }}>
              ...
              <text x={cx} y={cy + 10} textAnchor="middle" fill="var(--text-muted)" fontSize={7} fontFamily="JetBrains Mono">
                /100
              </text>
            </svg>
          </div>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ marginBottom: 6 }}>
              <div style={{ fontFamily: 'JetBrains Mono', fontSize: 8, color: 'var(--text-muted)', letterSpacing: '0.08em', marginBottom: 2 }}>SURFACE</div>
              <div style={{ fontFamily: 'JetBrains Mono', fontSize: 12, color: 'var(--text-primary)', fontWeight: 600, letterSpacing: '0.06em' }}>
                {road.surface.toUpperCase()}
              </div>
            </div>
            <div>
              <div style={{ fontFamily: 'JetBrains Mono', fontSize: 8, color: 'var(--text-muted)', letterSpacing: '0.08em', marginBottom: 2 }}>ROUGHNESS</div>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
            <span style={{ fontFamily: 'JetBrains Mono', fontSize: 8, color: 'var(--text-muted)', letterSpacing: '0.08em' }}>ROAD ROUGHNESS INDEX</span>
          </div>
        ...
        {/* Detected features */}
        <div>
          <div style={{ fontFamily: 'JetBrains Mono', fontSize: 8, color: 'var(--text-muted)', letterSpacing: '0.08em', marginBottom: 6 }}>
            DETECTED FEATURES
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            {road.features.map((f) => (
              <div key={f.id} style={{
                ...
                <span style={{ fontFamily: 'Inter', fontSize: 10, color: f.type === 'pothole' ? 'var(--warning)' : 'var(--text-label)', flex: 1 }}>
                  {f.label}
                </span>
                {f.distance > 0 && (
